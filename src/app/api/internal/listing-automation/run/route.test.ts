...
const m = vi.hoisted(() => ({ poll: vi.fn(), queue: vi.fn(), sync: vi.fn(), stockx: vi.fn(), bulk: vi.fn(), db: vi.fn(() => ({})) }));
...
describe("scheduled automation authentication", () => {
  beforeEach(() => { vi.resetAllMocks(); m.stockx.mockResolvedValue({ scheduled: 1 }); m.bulk.mockResolvedValue(1); vi.stubEnv("CRON_SECRET", "cron-test"); vi.stubEnv("INVENTORY_SYNC_WORKER_SECRET", "worker-test"); m.poll.mockResolvedValue({ checked: 1, failed: 0, deferred: 0 }); m.queue.mockResolvedValue(1); m.sync.mockImplementation(async () => Response.json({ ok: true })); });
  ...
  it("does not begin stages whose deadline has passed", async () => {
    ...
    const response = await GET(new Request("https://example.test", { headers: { authorization: "Bearer cron-test" } }));
    expect((await response.json()).deferredStages).toEqual(["stockx_schedule", "inventory_sync", "bulk_preparation", "listing_preparation"]);
