...
import { enqueueEtsyStatusChecks, ETSY_MONITOR_INTERVAL_MS, etsyMonitorKey } from "./etsy-status-monitor";
...
function fixture(candidates: Candidate[] = [candidate()]) {
  ...
  const typedDb = db as unknown as Parameters<typeof enqueueEtsyStatusChecks>[0];
...
describe("enqueueEtsyStatusChecks", () => {
  ...
  it("keeps eligibility, unsupported states and parked exclusions inside a bounded locked SQL selection", async () => {
    ...
    expect(query.values).toEqual(["production", "production", new Date(NOW - ETSY_MONITOR_INTERVAL_MS), "etsy-status-monitor-v1", String(Math.floor(NOW / ETSY_MONITOR_INTERVAL_MS)), 25]);
  });

  it("does not access the database when disabled or the deadline has elapsed", async () => {
    const f = fixture();
    expect(await enqueueEtsyStatusChecks(f.typedDb, { ...f.deps, enabled: () => false }, NOW + 10_000)).toMatchObject({ disabled: true, scheduled: 0 });
    expect(await enqueueEtsyStatusChecks(f.typedDb, f.deps, NOW)).toMatchObject({ deadlineReached: true, scheduled: 0 });
  ...
  it.each(["queued", "running", "retry_wait", "needs_review", "failed", "skipped", "canceled"] as const)("rechecks and preserves %s work after locking", async (status) => {
    ...
    const result = await enqueueEtsyStatusChecks(f.typedDb, f.deps, NOW + 10_000);
  ...
  it("does not treat an unrelated account's failed job as the listing's blocker", async () => {
    ...
    expect(await enqueueEtsyStatusChecks(f.typedDb, f.deps, NOW + 10_000)).toMatchObject({ scheduled: 1 });
  });
  ...
  it("still respects older pending review even if a newer successful check exists", async () => {
    ...
    expect(await enqueueEtsyStatusChecks(f.typedDb, f.deps, NOW + 10_000)).toMatchObject({ scheduled: 0 });
  });

  it("concurrent calls and adjacent windows never create two pending checks", async () => {
    const f = fixture();
    await Promise.all([
      enqueueEtsyStatusChecks(f.typedDb, f.deps, NOW + 10_000),
      enqueueEtsyStatusChecks(f.typedDb, { ...f.deps, now: () => NOW + ETSY_MONITOR_INTERVAL_MS }, NOW + ETSY_MONITOR_INTERVAL_MS + 10_000),
    ]);
    await enqueueEtsyStatusChecks(f.typedDb, { ...f.deps, now: () => NOW + ETSY_MONITOR_INTERVAL_MS }, NOW + ETSY_MONITOR_INTERVAL_MS + 10_000);
  ...
  it("does not reopen a completed check in the same window", async () => {
    ...
    expect(await enqueueEtsyStatusChecks(f.typedDb, f.deps, NOW + 10_000)).toMatchObject({ scheduled: 0, skipped: 1 });
    expect(f.jobs).toHaveLength(1);
  });
  ...
  it("bounds the batch and stops enqueueing when its shared deadline expires", async () => {
    ...
    let calls = 0;
    const result = await enqueueEtsyStatusChecks(f.typedDb, { ...f.deps, now: () => ++calls <= 2 ? NOW : NOW + 10_000 }, NOW + 5_000);
  ...
  it("rolls back and surfaces a sanitized failure instead of healthy counts", async () => {
    ...
    f.setFailEnqueue(2);
    await expect(enqueueEtsyStatusChecks(f.typedDb, f.deps, NOW + 10_000)).rejects.toThrow("Etsy sale checks could not be scheduled.");
