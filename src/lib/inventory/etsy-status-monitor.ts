...
import { getEtsyConfig } from "@/lib/marketplace/adapters/etsy/config";
...
export const ETSY_MONITOR_INTERVAL_MS = 5 * 60_000;
const BATCH_SIZE = 25;
const KEY_PREFIX = "etsy-status-monitor-v1";
...
export const etsyMonitorDeps = {
  enabled: () => { try { getEtsyConfig(); return true; } catch { return false; } },
  now: () => Date.now(),
};

export function etsyMonitorKey(listingId: string, window: number) {
 ...
 * reading Etsy. A queued check is never counted as a successful sale check. */
export async function enqueueEtsyStatusChecks(
  db: Db = getPrisma(),
  deps = etsyMonitorDeps,
  ...
  const dueBefore = new Date(now - ETSY_MONITOR_INTERVAL_MS);
  const window = Math.floor(now / ETSY_MONITOR_INTERVAL_MS);
  try {
    return await db.$transaction(async (tx) => {
      ...
      const candidates = await tx.$queryRaw<Candidate[]>(Prisma.sql`
        ...
        JOIN "MarketplaceConnection" c ON c."accountId" = i."accountId"
          AND c.marketplace = 'etsy' AND c.environment = ${ETSY_ENVIRONMENT}
        LEFT JOIN LATERAL (
        ...
        WHERE l.marketplace = 'etsy' AND l.environment = ${ETSY_ENVIRONMENT}
      ...
      for (const listing of candidates) {
        ...
        const job = await enqueueSyncJob(tx, {
          ...
          type: "detect_status",
          idempotencyKey: etsyMonitorKey(listing.id, window),
    ...
    throw new AppError("Etsy sale checks could not be scheduled. The next worker run will try scheduling again.", 503);
  }
}
