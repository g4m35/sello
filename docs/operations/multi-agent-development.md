...
- Cursor/Grok normally implements bounded frontend or feature tasks inside an assigned worktree.
...
The canonical clone is the repository/control point. It is used to inspect state, create worktrees, review, and integrate. Product implementation happens in contract-owned worktrees under `~/dev/`. `agent:start` fetches remote refs without moving a local integration branch, then creates the task branch from the current `origin/<base_branch>` commit.
