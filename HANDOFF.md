...
`b2fc862bafb296e9af35d688b2ce0a3438b55a3e`: `npx prisma validate`,
`git diff --check`, `npm run lint` (same two warnings in
`src/app/api/listings/draft/draft-actions.test.ts`), full `npm test` (209 files
/ 1371 tests), and `npm run build`. Worktree was clean and only `.env.example`
was present; no `.env*` secrets or `LOCAL_DEVELOPMENT_RULES.md` were found.
Vercel Production metadata still lists the required `STOCKX_*` names as
encrypted, but a temp-file `vercel env pull` outside the repo reported StockX
values empty/non-boolean, and a safe runtime callback probe returned
`503 STOCKX_NOT_ENABLED` for `STOCKX_API_ENABLED`. No secrets were printed; the
temp file/log were removed. Because Step 1 requires `STOCKX_API_ENABLED=true`
and non-empty StockX env values before OAuth/readiness, no StockX OAuth,
catalog search, market-data request, live listing, detect-status provider call,
delist/deactivate, paid checkout, Keychain access, cookie extraction, or bulk
action was run. Vercel log filter for deployment
`dpl_7VFFdP6jXEQpncBsPDE784uNYYhZ` returned no StockX/error/fatal/500/token/secret
matches.

2026-07-04 — Codex. Used subagents for the overnight StockX pass, fixed the
shared-account automation gap they found, pushed commit
`8f679a9110ee1cfac9453ecb12d25afe4003eaf7`, and deployed production
`dpl_7VFFdP6jXEQpncBsPDE784uNYYhZ`, READY and aliased to `https://sello.wtf`.
StockX/eBay adapter-backed delist jobs and StockX `detect_status` jobs now keep
account scope from the queued job, pass accountId into the marketplace handlers,
and restore SOLD status against the inventory owner when shared-account cleanup
runs as a teammate. StockX sold status reconciliation now passes
`inventoryOwnerUserId` into `markItemSold`. `.env.example` documents the
StockX runtime variable names with disabled/empty defaults only. Vercel lists
all required StockX Production env names as encrypted; values were not printed or
inspected. Validation: focused inventory/StockX suites passed (3 files / 53
tests); `npx prisma validate` passed; `git diff --check` passed; `npm run lint`
passed with only the two known warnings in
`src/app/api/listings/draft/draft-actions.test.ts`; full `npm test` passed (209
files / 1371 tests); `npm run build` passed locally and in Vercel. Production
smoke after deploy: `/`, `/channels`, and `/settings/marketplaces` returned
200; anonymous StockX status/connect/catalog/publish/generic publish/delist and
`/api/jobs` returned 401; Vercel error/fatal/500/StockX log filter returned no
matches. No live StockX listing, deactivate, provider status call, Keychain
access, cookie extraction, paid checkout, or bulk marketplace action was run.
...
marketplace page/status API so StockX no longer appears as a draft-only/no-sync
assisted channel when the API config is present. Production deployment
`dpl_9wySr8qrtFHA5E6GJawKVzKSGdEh` is READY and aliased to `https://sello.wtf`.
StockX live automation now has audited paths for dedicated
`/api/marketplaces/stockx/publish`, generic `/api/listings/publish`, generic
`/api/listings/delist`, and inventory-sync worker delist jobs. Guards remain:
full API config, connected StockX account, exact product/variant match, positive
price/quantity, no duplicate live listing, and explicit live confirmation.
Submitted StockX publish attempts persist as `RUNNING`/`LISTING` with operation
metadata and are not marked `LISTED` unless StockX reports active/success.
StockX deactivation marks `DELISTED` only after the official deactivate endpoint
confirms.

Validation: focused StockX/delist/queue/UI tests; focused `/api/jobs` and
capability label tests; full `npm test` (208 files / 1360 tests); `npm run
lint` (only the two known warnings in `draft-actions.test.ts`); `npm run build`;
`git diff --check`; diff secret-pattern scan found no credential values (only
placeholder test strings). Production smoke: `/`, `/channels`, and
`/settings/marketplaces` returned 200; anonymous StockX publish/delist/status
remained protected with 401; Vercel inspect showed READY aliases; Vercel error
log filter found no error/fatal/500 matches. Signed-in Chrome smoke reached
`https://sello.wtf/channels` and showed the new Sello-styled StockX copy, but
runtime StockX API readiness is still fail-closed because Vercel production
StockX variable names exist with empty values. Safe metadata checks printed only
presence/length/boolean state; no secret values were printed. Local ignored env
files checked in visible Sello checkouts also had zero `STOCKX_*` keys. A live
StockX listing/deactivate test is therefore blocked until real production
`STOCKX_CLIENT_ID`, `STOCKX_CLIENT_SECRET`, `STOCKX_API_KEY`, and the required
StockX config/secret values are populated in Vercel.
